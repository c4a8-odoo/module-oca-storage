...
-  When enabling ``Use As Default For Attachment`` you must be aware
   that this option applies from the point when the ``fs_attachment``
   has been loaded by Odoo. This means that some attachments (such as
   icons) created during the update of modules that are loaded before
